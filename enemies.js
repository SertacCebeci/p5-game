...
function spawnEnemy() {
  const p = game.player;
  const baseRadius = max(width, height) * 0.6;
  const spawnRadius = baseRadius + random(baseRadius * 0.6, baseRadius * 1.4);
  const angle = random(TWO_PI);
  const x = p.x + cos(angle) * spawnRadius;
  const y = p.y + sin(angle) * spawnRadius;
